...
$resp = Post-Form "$base/App/Account/Register" $s @{ CompanyName = '???? ??????? ???'; FullName = '??? ?????'; Email = $email; Password = 'Test@12345'; '__RequestVerificationToken' = (Get-Token $reg.Content) }
...
if ((Decode $resp) -match '??? ?? ???? ???????') { Write-Output "2. TRIAL BADGE OK" }
if ((Decode $resp) -match '????') { Write-Output "3. SEEDED MODULE ON DASHBOARD OK" }
...
$created = Post-Form "$base/App/m/leads/create" $s @{ f_name = '???? ?????'; f_company = '??????? ????'; f_phone = '09121234567'; f_email = 'maryam@pars.ir'; f_status = 'warm'; f_source = 'website'; f_description = '???? ?? ??? ????'; '__RequestVerificationToken' = (Get-Token $createPage.Content) }
if ((Decode $created) -match '???? ?????') { Write-Output "5. CREATE + LIST RENDER OK" } else { throw 'created record not in list' }
if ((Decode $created) -match '???') { Write-Output "6. PICKLIST BADGE OK" }
...
$invalid = Post-Form "$base/App/m/leads/create" $s @{ f_company = '???? ???'; '__RequestVerificationToken' = (Get-Token $createPage2.Content) }
if ((Decode $invalid) -match '?????? ???') { Write-Output "7. VALIDATION ERROR SHOWN OK" } else { throw 'validation message missing' }
...
$dup = Post-Form "$base/App/m/leads/create" $s @{ f_name = '??????'; f_phone = '09121234567'; '__RequestVerificationToken' = (Get-Token $createPage3.Content) }
if ((Decode $dup) -match '?? ??? ???? ????') { Write-Output "8. DUPLICATE CHECK OK" } else { throw 'duplicate not detected' }

# 6) Edit record
$editPage = Invoke-WebRequest -Uri "$base/App/m/leads/$recordId/edit" -WebSession $s -UseBasicParsing
$edited = Post-Form "$base/App/m/leads/$recordId/edit" $s @{ f_name = '???? ?????'; f_company = '??????? ???? ??'; f_phone = '09121234567'; f_status = 'hot'; '__RequestVerificationToken' = (Get-Token $editPage.Content) }
if ((Decode $edited) -match '??????? ???? ??') { Write-Output "9. EDIT OK" } else { throw 'edit failed' }

# 7) Search
$search = Invoke-WebRequest -Uri "$base/App/m/leads?q=%D9%85%D8%B1%DB%8C%D9%85" -WebSession $s -UseBasicParsing
if ((Decode $search) -match '???? ?????') { Write-Output "10. SEARCH OK" }
...
[System.IO.File]::WriteAllBytes($csvPath, [System.Text.Encoding]::UTF8.GetBytes("name,phone,status`n??? ?????,09351112233,cold`n???? ?????,09359998877,hot"))
...
$cookieJar = Join-Path $env:TEMP 'crm-curl-cookies.txt'
...
if ($importHtml -match '????? ???? ??' -and $importHtml -match '??? ?????') { Write-Output "12. CSV IMPORT OK" } else { Write-Output "12. CSV IMPORT FAILED" }
...
if ((Decode $del) -notmatch '???? ?????') { Write-Output "13. SOFT DELETE OK" }
$bin = Invoke-WebRequest -Uri "$base/App/recycle-bin" -WebSession $s -UseBasicParsing
if ((Decode $bin) -match '???? ?????') { Write-Output "14. RECYCLE BIN OK" }
...
if ((Decode $listAfter) -match '???? ?????') { Write-Output "15. RESTORE OK" }
...
$resp2 = Post-Form "$base/App/Account/Register" $s2 @{ CompanyName = '???? ???'; FullName = '??? ?????'; Email = $email2; Password = 'Test@12345'; '__RequestVerificationToken' = (Get-Token $reg2.Content) }
$list2 = Invoke-WebRequest -Uri "$base/App/m/leads" -WebSession $s2 -UseBasicParsing
if ((Decode $list2) -notmatch '???? ?????' -and (Decode $list2) -match '???? ?????? ??? ????') { Write-Output "16. TENANT ISOLATION OK" } else { throw 'TENANT LEAK!' }
